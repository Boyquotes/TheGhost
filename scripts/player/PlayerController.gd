...
@onready var voice_stream : AudioStreamPlayer3D = get_tree().get_first_node_in_group("VoiceStream")
...
var sprinting = false:
	set(value):
		...
		if value != old_sprinting:
			dashing_stop.emitting = true
			await get_tree().create_timer(0.5).timeout
			dashing_stop.emitting = false
			
		
var speed_bump = false:
	set(value):
		speed_bump = value
		if value:
			stamina -= 0.25
			voice_stream.playing = true
			voice_stream.play(0.0)
...
func _apply_movement():
	if on_floor and speed_bump and linear_velocity.length() < BUMP_MOD && is_move_input():
		walking_effects.emitting = true
		apply_central_impulse(Vector3(motion.x, 333.3, motion.z) * 1.5) 
	elif on_floor && linear_velocity.length() < MAX_SPEED && is_move_input():
		walking_effects.emitting = true
